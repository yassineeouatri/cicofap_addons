...
def get_query(self, args, operation, field, group_by=False, limit=False, apply_ir_rules=False):
    ...
    query_str = 'SELECT %s FROM ' % data + from_clause + join + where_str + multicompany_condition + group_by_str + ' order by value desc '+ limit_str
