...
class DashboardBlock(models.Model):
        ...
        for rec in dashboard_block:
            ...
            if rec.model_name:
                if rec.type == 'graph':
                    ...
                    total = '{:20,.2f}'.format(total).replace(',', ' ')
    ...
    @api.model
    def schedule_dashboard(self):
        self._cr.execute("""
            update dashboard_block a
                set filter = '[["date_order",">=","'||TO_CHAR(now(), 'YYYY-mm-01 00:00:00')||'"]]'
                from ir_act_client b  where b.id=a.client_action
                and  b.name like 'Mois en cours' and filter like '%"date_order"%';
            update dashboard_block a
                set filter = '[["order_id.date_order",">=","'||TO_CHAR(now(), 'YYYY-mm-01 00:00:00')||'"]]'
                from ir_act_client b where b.id=a.client_action
                and  b.name like 'Mois en cours' and filter like '%"order_id.date_order"%';
            update dashboard_block a
                set filter = '[["date_approve",">=","'||TO_CHAR(now(), 'YYYY-mm-01 00:00:00')||'"]]'
                from ir_act_client b  where b.id=a.client_action
                and  b.name like 'Mois en cours' and filter like '%"date_approve"%';
            
            update dashboard_block a
                set filter = '[["date_order",">=","'||TO_CHAR(now(), 'YYYY-01-01 00:00:00')||'"]]'
                from ir_act_client b  where b.id=a.client_action
                and  b.name like 'Année en cours' and filter like '%"date_order"%';
            update dashboard_block a
                set filter = '[["order_id.date_order",">=","'||TO_CHAR(now(), 'YYYY-01-01 00:00:00')||'"]]'
                from ir_act_client b where b.id=a.client_action
                and  b.name like 'Année en cours' and filter like '%"order_id.date_order"%';
            update dashboard_block a
                set filter = '[["date_approve",">=","'||TO_CHAR(now(), 'YYYY-01-01 00:00:00')||'"]]'
                from ir_act_client b  where b.id=a.client_action
                and  b.name like 'Année en cours' and filter like '%"date_approve"%';
        """)
